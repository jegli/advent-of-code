...
test('if a longer addition opcode works', () => {
	expect(runIntcodeProgram([1,9,10,3,2,3,11,0,99,30,40,50])).toStrictEqual([3500,9,10,70,2,3,11,0,99,30,40,50]);
});
...
test('run the intcode-program', () => {
	...
	console.log('value at postion 0 after program ran: ', result[0]);
})
	...
	for (let noun = 0; noun < 100; noun++) {
		for (let verb = 0; verb < 100; verb++) {
			...
			if (result[0] === valueToBeAtAddressOne) {
				return console.log('result is: ', 100 * noun + verb);
			}
		}
	}
});
